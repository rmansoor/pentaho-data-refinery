...
public class JobEntryDatasourcePublish extends JobEntryBase implements Cloneable, JobEntryInterface {
  ...
  public DatasourcePublishService getDatasourcePublishService () {
    if ( datasourcePublishService == null ) {
      datasourcePublishService = new DatasourcePublishService( log );
    }
    return datasourcePublishService;
  }
  ...
  public Result execute( Result result, int i ) throws KettleException {
    ...
    try {
      ...
      if ( databaseMeta != null ) {
        ...
        getDatasourcePublishService().publishDatabaseMeta( modelServerPublish, databaseMeta, forceOverride );
      } else {
      ...
      if ( dswFlag != null && dswFlag.equalsIgnoreCase( "true" ) ) {
        getDatasourcePublishService().publishDswXmi( modelName, xmiString, modelServerPublish, forceOverride );
      } else {
        getDatasourcePublishService().publishMetadataXmi( modelName, xmiString, modelServerPublish, forceOverride );
      }
      metaPublished = true;
      ...
      if ( dsPublished && databaseMeta != null ) {
        getDatasourcePublishService().deleteDatabaseMeta( modelServerPublish, databaseMeta );
      }
      if ( metaPublished && modelName != null ) {
        getDatasourcePublishService().deleteXMI( modelServerPublish, modelName, dswFlag );
      }
